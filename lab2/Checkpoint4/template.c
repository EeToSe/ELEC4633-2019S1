...
int u_to_daq (int u) { //writing speed
	int write_value = 2047;
	if (u<= -180){
		u = u + 360;
	}
	if (u>=180){
...
void thread_code(long arg)
{
  while (1)
  {
    ...
    rtf_get(1, &setpoint, sizeof(int));
	int error = SETPOINT1 - result;
	int u = error*KP; //degrees
	
    /* To write data to a D/A converter on the DAQ */
    comedi_data_write(comedi_dev, WRITE_SUBDEVICE, WRITE_CHANNEL, RANGE, AREF, u_to_daq(u));
	printk("u = %d write_value = %d\n\n", u, u_to_daq(u));
...
static int __init template_init(void)
{
  ...
  rtf_create(0, 10*sizeof(int)); //to hold 10 ints (data to send)
  rtf_create(1, sizeof(int)); 	 //to hold 1 int (data to receive)
